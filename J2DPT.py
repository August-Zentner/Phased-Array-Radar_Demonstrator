...
from PySide6.QtWidgets import QApplication  # Import QApplication from PySide6.QtWidgets
from PySide6.QtGui import QFont  # Correct import for QFont in PySide6
...
wavelength = 3E8/rx_wave              # wavelength of the RF carrier plus modulation
...
def send_data(t_phase_delay):    
    ...
    iq0 = i0 + 1j * q0
    iq1 = (i0 + 1j * q0) * np.exp(1j * np.deg2rad(t_phase_delay + t_phase_cal)) 
...
def scan_for_DOA():
    ...
    for phase_delay in delay_phases:  
        ...
        peak_delta = []
        monopulse_phase = []
        delayed_Rx_1 = Rx_1 * np.exp(1j*np.deg2rad(phase_delay+r_phase_cal))
        delayed_sum = Rx_0 + delayed_Rx_1
        delayed_delta = Rx_0 - delayed_Rx_1
        delayed_sum_dbfs = dbfs(delayed_sum)
        delayed_delta_dbfs = dbfs(delayed_delta)
        mono_angle = calcTheta(phase_delay)
...
def Tracking(last_delay):
    ...
    send_data(last_delay)
    data = sdr.rx()
    Rx_0=data[0]
    Rx_1=data[1]
    delayed_Rx_1 = Rx_1 * np.exp(1j*np.deg2rad(last_delay+r_phase_cal))
    delayed_sum = Rx_0 + delayed_Rx_1
    delayed_delta = Rx_0 - delayed_Rx_1
    delayed_sum_fft, delayed_sum_dbfs = dbfs(delayed_sum)
    delayed_delta_fft, delayed_delta_dbfs = dbfs(delayed_delta)
    ...
    if np.sign(mono_angle) > 0:
        ...
        if calcTheta(new_delay) > 80 or calcTheta(new_delay) < -80:
...
delay = 0#peak_delay  # this will be the starting point if we are doing monopulse tracking
...
if __name__ == '__main__':
    app = QApplication.instance()  # Use existing QApplication instance if it exists
    if app is None:
